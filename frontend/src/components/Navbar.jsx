                    ...
                    <div className="collapse navbar-collapse" id="navbarSupportedContent" ref={navbarCollapse}>
                        <ul className="navbar-nav me-auto mb-2 mb-lg-0">
                            <li className="nav-item mx-2">
                                <Link className={`nav-link ${location.pathname === '/' ? 'active' : darkMode ? "non-active-light": "non-active-dark"}`} aria-current="page" to="/" onClick={handleClick}>Home</Link>
                            </li>
                            {localStorage.getItem('auth-token') && (
                                <li className="nav-item mx-2">
                                    <Link className={`nav-link ${location.pathname === '/createpost' ? 'active' : darkMode ? "non-active-light": "non-active-dark"}`} to="/createpost" onClick={handleClick}>Create Post</Link>
                                </li>
                            )}
                            <li className="nav-item mx-2">
                                <Link className={`nav-link ${location.pathname === '/posts' ? 'active' : darkMode ? "non-active-light": "non-active-dark"}`} to="/posts" onClick={handleClick}>Posts</Link>
                            </li>
                            <li className="nav-item mx-2">
                                <Link className={`nav-link ${location.pathname === '/about' ? 'active' : darkMode ? "non-active-light": "non-active-dark"}`} to="/about" onClick={handleClick}>About</Link>
                            </li>
                        </ul>
                        {!localStorage.getItem('auth-token') ? (
                            <>
