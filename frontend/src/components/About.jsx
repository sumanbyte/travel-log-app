...
const About = () => {
  ...
  return (
    <div className={`pt-5 ${darkMode ? "dark-mode" : ""} dark-mode-transition`}>
      <div className={`container`}>
        ...
        <div className="d-flex align-items-center justify-content-between flex-wrap">
          ...
          <div className="d-flex dev-info" style={{ gap: "30px" }}>
            ...
            ><FaGithub color={`${darkMode ? "white": "black"}`} size={25} />
            </Link>
            <Link to={'https://www.linkedin.com/in/sumanbyte/'} target="_blank">
              <FaLinkedin color={`${darkMode ? "white": "black"}`} size={25} />
            </Link>
            <Link to={'https://medium.com/@sumanbyte'} target="_blank">
              <FaMedium color={`${darkMode ? "white": "black"}`} size={25} />
            </Link>
          </div>
        </div>
        <div style={{paddingBottom: "20px"}}>
        <hr color={`${darkMode ? "white": "black"}`} style={{marginBottom: 0}} />
        </div>
      </div>
    </div>
  )
